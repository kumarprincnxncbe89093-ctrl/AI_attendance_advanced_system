...
def student_dashboard():
    ...
    with st.spinner('Loading your attendance profile..'):
        try:
    ...
    attended=sum(item["attended"] for item in stats_map.values())
    attendance_rate=f"{round((attended/total_classes)*100)}%" if total_classes else "0%"

    st.markdown(
        """
        <div class="ai-hero">
            <div class="ai-eyebrow">Student workspace</div>
            <h2>Attendance Profile</h2>
            <p>Track enrolled subjects, attendance percentage, and face or voice biometric profile updates.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    render_metric_grid([
        ("Subjects", len(visible_subjects), "Currently enrolled courses"),
        ("Classes", total_classes, "Attendance sessions logged"),
        ("Attended", attended, "Present records captured"),
        ("Attendance", attendance_rate, "Overall percentage"),
    ])
    ...
    tabs=[
        ("subjects","Subjects",":material/menu_book:"),
        ("profile","Biometrics",":material/fingerprint:"),
    ]
    ...
    for index,(tab_key,label,icon) in enumerate(tabs):
        with nav_cols[index]:
            button_type="primary" if st.session_state.current_student_tab==tab_key else "tertiary"
            if st.button(label,type=button_type,width="stretch",icon=icon,key=f"student_tab_{tab_key}"):
                st.session_state.current_student_tab=tab_key
                st.rerun()

    st.divider()

    if st.session_state.current_student_tab=="profile":
    ...
    if not visible_subjects:
        ...
        st.markdown(
            f"""
            <div class="ai-hero subject-overall-card">
                <div class="ai-eyebrow">Overall listed attendance</div>
                <h2>{attendance_rate}</h2>
                <p>{attended} attended out of {total_classes} total class records across {len(visible_subjects)} subject(s).</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    for i,sub in enumerate(visible_subjects):
        ...
        subject_percent=round((stats["attended"]/stats["total"])*100) if stats["total"] else 0
        ...
        with cols[i % 2]:
            subject_card(
                ...
                stats=[
                    ...
                    ('Present','Attended',stats['attended']),
                    ('Rate','Percentage',f"{subject_percent}%"),
                ],
                progress_percent=subject_percent,
                progress_label="Subject Attendance",
...
def student_screen():
    ...
    st.markdown(
        """
        <div class="ai-hero">
            <div class="ai-eyebrow">Face recognition access</div>
            <h2>Student Workspace</h2>
            <p>Look into the camera to match your face embedding and open your attendance profile.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if show_registration:
        with st.container(border=True):
            st.markdown(
                """
                <div class="ai-hero">
                    <div class="ai-eyebrow">New embedding profile</div>
                    <h2>Register Student</h2>
                    <p>Create a face profile and optionally add a voice profile for voice attendance.</p>
                </div>
                """,
                unsafe_allow_html=True,
            )
