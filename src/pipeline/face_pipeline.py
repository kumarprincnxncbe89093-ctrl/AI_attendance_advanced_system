...
def _face_recognition_ready():
    ...
    st.error(
        "Face recognition dependency is missing. Install `dlib`/`dlib-bin` "
        "in this Python environment, then restart Streamlit."
    )
    return False
...
def get_face_embeddings(image_np):
    ...
    faces=detector(image_np,1) # test with 2 or 3 

    encodings=[]

    for face in faces:
        shape=sp(image_np,face)
        face_descriptor=facerec.compute_face_descriptor(image_np,shape,1)  ## test with 2 or 3   or 128 embedding 
